		...
		<div
			...
			<nav
				...
				${loginID}님 안녕하세요 <!-- 강사님께 질문 2-2 -->
						...
						<!-- 강사님께 질문 3 -->
						<c:if test="${seqID < 0 }"><a href="dash.admin">관리자페이지</a></c:if>
						<c:if test="${seqID > 0 }"><a href="#.mem">마이페이지</a></c:if>
					</c:when>
					<c:otherwise>
...
<div class="flex justify-center" id="hashtag">
 <span class="searchicon"></span>
	<select id="selectTab" class="select_tab" title="세부검색">
	<option value="x">-검색</option>
	<option value="0" id="tab1">
		지역
	</option>
	<option value="1">
		음식종류
   ...
   <span class=" mt-2"  id="mapo">
    ...
    <span class=" mt-2" class="food" id="boonsik">
    <button>#분식</button>
    </span> &nbsp;&nbsp;
   <span class=" mt-2" class="food" id="japaneesefood">
    <button>#일식</button>
    </span> &nbsp;&nbsp;
   	<span class=" mt-2" class="food" id="cafe">
    <button>#카페</button>
    </span> &nbsp;&nbsp;
    <span class=" mt-2" class="food" id="fastfood">
    <button>#간식</button>
    </span> 
...
회원가입 , 로그인 : 곽서호 , 김도경 <br><br>

마이페이지 : 서호현 , 장건희	<br><br>

맛집 리스트 : 서호현 , 김도경	<br><br>

검색 : 곽서호 , 임도혁 	<br><br>

맛집 게시판 :  임도혁 , 곽서호 , 장건희	<br><br>

반려견 게시판 : 김도경 , 서호현	<br><br>
...
관리 : 임도혁
      ...
      <div class="">
        ...
        <p class="mb-4">
         <a href="signin.mem" class="text-gray-600"> 로그인 </a>, <a href="signup.mem" class="text-gray-600">회원가입</a>
        </p>
        <p class="mb-4">
        ...
        <p>
          <a href="/fboard.rest?cpage=1" class="text-gray-600">맛집 </a>, <a href="/list.board" class="text-gray-600"> 애견 게시판</a>
        </p>
      </div>
      <div class="">
        <h6 class="uppercase font-semibold mb-4 flex justify-center md:justify-start">
...
// 클릭시 검색창에 #xx 보이게 하기
	$("#koreanfood").on("click",function(){
		...
		if($("#searching").val()==""||$("#searching").val()=="#한식"||$("#searching").val()=="#일식"||$("#searching").val()=="#분식"||$("#searching").val()=="#카페"||$("#searching").val()=="#간식"){
		...
		//다른 카테고리가 이미 있으면 그 뒤에 값을 + 해준다
		}else if($("#searching").val()!=""||$("#searching").val()!="#한식"||$("#searching").val()!="#일식"||$("#searching").val()!="#분식"||$("#searching").val()!="#카페"||$("#searching").val()!="#간식"){
			   ...
			   if(korean == (newtext)){
			    ...
			    else if(korean=="#일식"||korean=="#카페"||korean=="#분식"||korean=="#간식"){
	...
	$("#japaneesefood").on("click",function(){
		...
		//다른 카테고리가 이미 있으면 그 뒤에 값을 + 해준다
			   ...
			   if(japan == (newtext)){
			    ...
			    else if(japan=="#한식"||japan=="#카페"||japan=="#분식"||japan=="#간식"){
			    	let first = elem.substring(elem.length-8,elem.length-4);
			    	let seconed = elem.substring(elem.length-3,elem.length);
			    	let result = first+" "+newtext;
			    	$("#searching").val(result);
	...
	$("#boonsik").on("click",function(){
		...
		//다른 카테고리가 이미 있으면 그 뒤에 값을 + 해준다
			   ...
			   if(boonsik == (newtext)){
			    ...
			    else if(boonsik=="#한식"||boonsik=="#카페"||boonsik=="#일식"||boonsik=="#간식"){
			    	let first = elem.substring(elem.length-8,elem.length-4);
			    	let seconed = elem.substring(elem.length-3,elem.length);
			    	let result = first+" "+newtext;
			    	$("#searching").val(result);
	...
	$("#cafe").on("click",function(){
		...
		//다른 카테고리가 이미 있으면 그 뒤에 값을 + 해준다
			   ...
			   if(cafe == (newtext)){
			    ...
			    else if(cafe=="#한식"||cafe=="#일식"||cafe=="#분식"||cafe=="#간식"){
			    	let first = elem.substring(elem.length-8,elem.length-4);
			    	let seconed = elem.substring(elem.length-3,elem.length);
			    	let result = first+" "+newtext;
			    	$("#searching").val(result);
	...
	$("#fastfood").on("click",function(){
		...
		if($("#searching").val()==""||$("#searching").val()=="#한식"||$("#searching").val()=="#일식"||$("#searching").val()=="#분식"||$("#searching").val()=="#카페"||$("#searching").val()=="#간식"){
			...
			$("#searching").val("#간식");
			 document.getElementById("searchbytagName").setAttribute("href", "/fboard.rest?cpage=1&category=all&target=간식");
		//다른 카테고리가 이미 있으면 그 뒤에 값을 + 해준다
		}else if($("#searching").val()!=""||$("#searching").val()!="#한식"||$("#searching").val()!="#일식"||$("#searching").val()!="#분식"||$("#searching").val()!="#카페"||$("#searching").val()!="#간식"){
			   let elem = $("#searching").val();
			    let newtext = "#간식";
